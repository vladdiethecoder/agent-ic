...
import { activatePolicyVersion, createPolicyVersion, diffPolicyVersions, getActivePolicyVersion, getPolicyVersion, listPolicyVersions, evaluatePolicy } from '../../../lib/policyStore.js';
...
export async function POST(request) {
  ...
  const permission = action === 'evaluate' ? 'view_evidence' : 'manage_policy';
  ...
  if (action === 'evaluate') {
    ...
    const evaluation = evaluatePolicy({ policy: envelope, attemptedAction: body.attemptedAction || {} });
    return NextResponse.json({ auth: authContext(access.principal), evaluation });
  }

  return jsonError(400, 'invalid_action', 'Use action: create, activate, or evaluate');
}
