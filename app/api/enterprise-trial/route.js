...
import { EnterpriseTrialResultSchema, parseSchema } from '../../../lib/schemas.js';
...
export async function POST(request) {
  ...
  const requireLiveProof = productionMode || body.requireLiveProof === true || process.env.AGENT_IC_REQUIRE_LIVE_PROOF === 'true';
  const tenantScope = requireTenantScope(access.principal, tenantFromBody(body));
  if (!tenantScope.ok) return tenantScope.response;

  if (productionMode && !caseId) {
  ...
  const approvalResult = productionMode
  ...
  const idempotencyFingerprint = fingerprintPayload({ tenantId: access.principal.tenantId, caseId, missionStatement, approvalId: body.approvalId || '', requireLiveProof, production: productionMode });
  ...
  try {
    ...
    const result = await runEnterpriseTrial({
      ...
      caseId,
      requireLiveProof,
      approvalContext: { required: productionMode, approval: approvalResult.approval || null },
      integrations,
      hooks: {
    ...
    const parsedResult = parseSchema(EnterpriseTrialResultSchema, result);
    if (!parsedResult.ok) {
      throw new Error(`Trial result schema invalid: ${parsedResult.error.issues?.[0]?.message || 'invalid result'}`);
    }
    ...
    appendAudit({
      ...
      spendApprovalStatus: result.spendApproval?.status || null,
      productionAccessApproved: result.productionAccessDecision?.approved === true,
      productionAccessStatus: result.productionAccessDecision?.status || null,
      productionAccessScope: result.productionAccessDecision?.scope || null,
      productionAccessBlockers: Array.isArray(result.productionAccessDecision?.blockers)
        ? result.productionAccessDecision.blockers.slice(0, 5)
        : [],
    });
    ...
    const message = error?.message?.slice(0, 300) || 'Trial failed';
    const strictProofMissing = /Strict provider proof required/i.test(message);
    incrementCounter('agent_ic_trials_failed_total', {
      tenantId: access.principal.tenantId,
      caseId: caseId || 'unknown',
      reason: strictProofMissing ? 'strict_provider_proof_missing' : 'trial_failed',
    });
    logError(strictProofMissing ? 'strict_provider_proof_missing' : 'enterprise_trial_failed', error, { tenantId: access.principal.tenantId, caseId });
    if (strictProofMissing) {
      const missingProof = missingProofFromStrictError(message);
      appendAudit({
        ...authContext(access.principal),
        actor: 'Agent IC',
        kind: 'trial',
        action: 'enterprise_trial_blocked_strict_proof_missing',
        caseId,
        detail: message,
        strictProofMissing: missingProof,
      });
      const responseBody = {
        error: message,
    ...
    return jsonError(500, 'trial_failed', message);
  }
}

function missingProofFromStrictError(message = '') {
  const [, tail = ''] = String(message).split('missing:');
  return tail
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean)
