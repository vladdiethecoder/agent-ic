...
export async function GET(request) {
  ...
  return NextResponse.json({
    ...
    proofSurfaces: {
      ...
      policy: openshellAvailable
        ? 'OpenShell sandbox policy available; policy adapter still records the 403 receipt'
        : 'Policy adapter records the deny-by-default 403 receipt when OpenShell is unavailable',
      model: providers.nemotron.state === 'live'
        ? 'NVIDIA Nemotron configured for sample classification and synthesis'
    ...
    providers: {
      ...
      policy: {
        state: openshellAvailable ? 'live' : providers.nemoclaw.state,
        mode: openshellAvailable ? 'openshell' : providers.nemoclaw.mode,
        detail: openshellAvailable ? 'OpenShell binary/status available' : providers.nemoclaw.detail,
      },
    },
