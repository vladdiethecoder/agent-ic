...
- Video QA: `demo-out/video-qa-report-winning-v3.json` - PASS, 65/65 checks using `ffprobe`/`ffmpeg` video analysis plus ImageMagick contact-sheet and frame-difference analysis. OCR is skipped by default and recorded only as a diagnostic when explicitly enabled.
