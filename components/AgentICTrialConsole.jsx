...
function TrialResult({ result, onReset }) {
  ...
  const spendApproval = result.spendApproval || {};
  const approvalRequired = spendApproval.required === true;
  const approvalValue = approvalRequired
    ? String(spendApproval.status || 'missing').replace(/_/g, ' ')
    : 'local trial only';
  const approvalDetail = approvalRequired
    ? `${spendApproval.decidedByRole || 'approver'} · cap ${spendApproval.spendCap ? money.format(spendApproval.spendCap) : money.format(spendCap)}`
    : 'production expansion still requires finance approval';
  ...
  const productionAccess = result.productionAccessDecision || {};
  const productionAccessStatus = productionAccess.approved === true ? 'Approved' : 'Not approved';
  const productionAccessDetail = productionAccess.approved === true
    ? productionAccess.recommendedAction || 'Scoped production access approved under policy.'
    : `${productionAccess.scope || 'governed_trial_only'} · ${(productionAccess.blockers || []).slice(0, 2).join('; ') || 'production access requires approval and live receipts'}`;
      ...
      <div className="ic-panel ic-trial-full ic-provider-receipts">
        ...
        <div className="ic-panel-body">
          ...
          <div className="ic-provider-proof-grid">
            ...
            <ReceiptCard label="Approval" value={approvalValue} detail={approvalDetail} tone={approvalRequired && spendApproval.status !== 'approved' ? 'blocked' : 'neutral'} />
            ...
            <ReceiptCard label="Production access" value={productionAccessStatus} detail={productionAccessDetail} tone={productionAccess.approved === true ? 'positive' : 'blocked'} />
          </div>
        </div>
      </div>
      ...
      <div className="ic-panel ic-trial-full ic-renewal-proof">
        ...
        <div className="ic-panel-body">
          ...
          <div className="ic-ledger-proof-grid">
            ...
            <ReceiptCard label="Production access" value={productionAccessStatus} detail={productionAccess.scope || 'governed_trial_only'} tone={productionAccess.approved === true ? 'positive' : 'blocked'} />
          </div>
