...
export default function AgentICAdminConsole() {
  ...
  async function evaluatePolicy(id) {
    await mutate('Evaluated policy block', () => api('/api/policies', {
      method: 'POST',
      body: JSON.stringify({ action: 'evaluate', tenantId, policyId: id, attemptedAction: { name: 'CARFAX vehicle-history report', attemptedAmount: 150 } }),
    }));
  }
    ...
    <main className="admin-shell">
      ...
      <section className="admin-grid top-grid">
        ...
        <Panel title="Readiness + proof">
          ...
          <div className="fact-list">
            ...
            <span>Readiness scope: {ready?.readinessScope || 'unknown'}</span>
            <span>Production deployment cleared: {ready?.productionReady ? 'yes' : 'no'}</span>
          ...
          <p className="caveat">{ready?.truthModel || 'Full production deployment requires enterprise SSO/JWKS, durable DB/WORM storage, deployed observability, and compliance evidence.'}</p>
        </Panel>
      </section>
      ...
      <section className="admin-grid">
        ...
        <Panel title="Payment Events">
          ...
          <input value={reconcileSessionId} onChange={(e) => setReconcileSessionId(e.target.value)} placeholder="Checkout Session ID" aria-label="Stripe Checkout Session ID" />
