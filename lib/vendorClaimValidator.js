...
export function validateVendorClaims(claims, measuredResults) {
  ...
  for (const claim of claims) {
    ...
    if (!validation.passed) {
      // Critical claims that fall short by >15 points trigger KILL
      const shortfall = validation.shortfall || 0;
      verdict = shortfall > 15 ? 'failed' : 'partially_met';
    }
    ...
    results.push({
      ...
      decisionImpact: !validation.passed && (validation.shortfall > 15) ? 'KILL' : null,
    });
  }
