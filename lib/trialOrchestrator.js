...
export async function runEnterpriseTrial({
  ...
  let playbook = generatePlaybook(caseDef, decision, trialContext);
  ...
  if (requireLiveProof && !hermesExecutionReceipt?.ok) {
    throw new Error(`Live Hermes proof required but unavailable: ${hermesExecutionReceipt?.error || 'missing receipt'}`);
  }
