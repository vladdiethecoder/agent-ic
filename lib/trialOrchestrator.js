...
export async function runEnterpriseTrial({
  ...
  hooks = {},
  recordMode = false,
  approvalContext = { required: false, approval: null },
  ...
  if (integrations.hermes?.dispatch) {
    ...
    try {
      ...
      if (hermesExecutionReceipt.receiptVerified) {
        playbook = {
          ...playbook,
          hermesNative: true,
      ...
      onTrace?.('hermes.dispatch.response', {
        ...
        receiptVerified: hermesExecutionReceipt.receiptVerified,
  ...
  onTrace?.('playbook.generated', { name: playbook.name, steps: playbook.steps.length, hermesLive: hermesExecutionReceipt?.receiptVerified === true });
  ...
  const spendApproval = normalizeSpendApproval(approvalContext);
  const productionAccessDecision = evaluateProductionAccessDecision({
    caseDef,
    finalDecision,
    decision,
    trialEvidence,
...
function normalizeSpendApproval(approvalContext = {}) {
  const approval = approvalContext.approval || null;
  return {
    required: approvalContext.required === true,
    status: approval?.status || (approvalContext.required === true ? 'missing' : 'not_required_for_local_trial'),
    id: approval?.id || null,
    spendCap: Number.isFinite(Number(approval?.spendCap)) ? Number(approval.spendCap) : null,
    caseId: approval?.caseId || null,
    decidedAt: approval?.decidedAt || null,
    decidedByRole: approval?.decidedByRole || null,
    evidence: approval
      ? 'approved spend envelope matched before trial execution'
      : approvalContext.required === true
        ? 'approval required but not present'
        : 'local trial run; production expansion still requires approval',
  };
}


function evaluateProductionAccessDecision({ caseDef, finalDecision, decision, trialEvidence, evidenceArtifacts, policyBlockResult, stripeResult, spendApproval, hermesExecutionReceipt }) {
  const blockers = [];
  const classification = trialEvidence?.classificationMethod || {};
  const liveClassificationReceipt = hasVerifiedNemotronClassification(trialEvidence);
  const openShellVerified = hasVerifiedOpenShellPolicyReceipt(policyBlockResult);
  const hermesRecorded = hasVerifiedHermesReceipt(hermesExecutionReceipt);
  const liveSpendReceipt = hasVerifiedLiveStripeReceipt(stripeResult);
  const spendApprovalVerified = hasVerifiedSpendApproval(spendApproval, caseDef);
  const evidenceArtifactsVerified = hasVerifiedEvidenceArtifacts(evidenceArtifacts);

  if (finalDecision?.verdict !== 'CONTINUE') blockers.push(`procurement verdict is ${finalDecision?.verdict || 'missing'}`);
  if (decision?.evidence?.completeness?.blocking) blockers.push(`evidence incomplete: ${decision.evidence.completeness.reasons?.join('; ') || 'required receipt missing'}`);
  if (!evidenceArtifactsVerified) blockers.push('evidence artifacts missing or unverified');
  if (!liveClassificationReceipt) blockers.push('Nemotron classification receipt missing');
  if (!openShellVerified) blockers.push('verified OpenShell 403 policy receipt missing');
  if (!hermesRecorded) blockers.push('Hermes dispatch receipt missing');
  if (!spendApprovalVerified) blockers.push('production spend/access approval missing');
  if (!liveSpendReceipt) blockers.push('live-mode spend receipt missing');
  if (trialEvidence?.blockedActionBypassed) blockers.push('policy bypass recorded');

  const approved = blockers.length === 0;
  return {
    approved,
    status: approved ? 'approved' : 'not_approved',
    scope: approved ? 'scoped_production_access' : 'governed_trial_only',
    recommendedAction: approved
      ? 'grant scoped production access under the recorded policy envelope'
      : 'keep vendor agent in governed trial or renewal-review state until blockers clear',
    blockers,
    evidence: {
      verdict: finalDecision?.verdict || null,
      approvalStatus: spendApproval?.status || null,
      approvalRequired: spendApproval?.required === true,
      approvalVerified: spendApprovalVerified,
      evidenceArtifactsVerified,
      stripeMode: stripeResult?.mode || 'unavailable',
      stripeTestMode: String(stripeResult?.checkout?.id || '').startsWith('cs_test'),
      stripeLiveReceiptVerified: liveSpendReceipt,
      nemotronClassificationReceipt: liveClassificationReceipt,
      openShellVerified403: openShellVerified,
      hermesReceipt: hermesRecorded,
      hermesReceiptVerified: hermesRecorded,
      policyBypass: trialEvidence?.blockedActionBypassed === true,
    },
  };
}

function hasVerifiedEvidenceArtifacts(evidenceArtifacts = []) {
  if (!Array.isArray(evidenceArtifacts)) return false;
  const requiredKinds = new Set(['trial-evidence', 'worker-results']);
  const seen = new Set();
  for (const artifact of evidenceArtifacts) {
    if (
      requiredKinds.has(artifact?.kind) &&
      typeof artifact?.artifactId === 'string' &&
      artifact.artifactId.startsWith(`evid_${artifact.kind}_`) &&
      /^[a-f0-9]{64}$/i.test(String(artifact?.sha256 || '')) &&
      Number(artifact?.bytes || 0) > 0 &&
      typeof artifact?.path === 'string' &&
      artifact.path.includes(String(artifact?.runId || 'missing-run'))
    ) {
      seen.add(artifact.kind);
    }
  }
  return requiredKinds.size === seen.size;
}

function hasVerifiedSpendApproval(spendApproval = {}, caseDef = {}) {
  const approvalId = String(spendApproval?.id || spendApproval?.idMasked || '');
  const spendCap = Number(spendApproval?.spendCap);
  const requiredCap = Number(caseDef?.policyEnvelope?.spendCap || 0);
  const decidedAtMs = Date.parse(String(spendApproval?.decidedAt || ''));
  return Boolean(
    spendApproval?.required === true &&
    spendApproval?.status === 'approved' &&
    /^appr_[A-Za-z0-9._:-]{8,}$/.test(approvalId) &&
    spendApproval?.caseId === caseDef?.id &&
    Number.isFinite(spendCap) &&
    spendCap >= requiredCap &&
    Number.isFinite(decidedAtMs) &&
    spendApproval?.decidedByRole === 'finance_approver'
  );
}

function enforceStrictProviderProof({ stripeResult, trialEvidence, policyBlockResult, hermesExecutionReceipt }) {
  const missing = [];
  if (!hasVerifiedLiveStripeReceipt(stripeResult)) {
    missing.push('Stripe live-mode Checkout receipt');
  }

  if (!hasVerifiedNemotronClassification(trialEvidence)) {
    missing.push('Nemotron classification request id');
  }

  if (!hasVerifiedOpenShellPolicyReceipt(policyBlockResult)) {
    missing.push('OpenShell verified 403 policy receipt');
  }

  if (!hasVerifiedHermesReceipt(hermesExecutionReceipt)) {
...
function hasVerifiedOpenShellPolicyReceipt(policyBlockResult = {}) {
  const proof = policyBlockResult?.proof || {};
  return Boolean(
    ...
    policyBlockResult?.enforcementType === 'container-network-policy' &&
    typeof policyBlockResult?.sandbox === 'string' &&
    policyBlockResult.sandbox.length > 0 &&
    /^openshell-block-\d+/.test(String(policyBlockResult?.receipt || '')) &&
...
function hasVerifiedNemotronClassification(trialEvidence = {}) {
  const classification = trialEvidence?.classificationMethod || {};
  const requestId = String(classification.nemotronRequestId || '');
  ...
  const casesProcessed = Number(trialEvidence?.casesProcessed || 0);
...
function hasVerifiedLiveStripeReceipt(stripeResult = {}) {
  const checkout = stripeResult?.checkout || {};
  const retrieval = stripeResult?.retrieval || null;
  const sessionId = checkout.id || stripeResult?.sessionId || null;
  const spendCapDollars = Number(stripeResult?.spendCapDollars);
  const retrievedAmount = Number(retrieval?.amount_total ?? checkout?.amount_total);
  const expectedAmount = Math.round(spendCapDollars * 100);
  const amountMatches = Number.isFinite(spendCapDollars) && spendCapDollars > 0
    ? Number.isFinite(retrievedAmount) && retrievedAmount === expectedAmount
    : true;

  return Boolean(
    sessionId &&
    String(sessionId).startsWith('cs_live') &&
    stripeResult?.mode === 'live' &&
    checkout?.livemode === true &&
    retrieval?.livemode === true &&
    retrieval?.id === sessionId &&
    amountMatches
  );
}

function hasVerifiedHermesReceipt(receipt = {}) {
  const liveSources = new Set(['nemohermes-sandbox', 'hermes-gateway', 'hermes-cli']);
  if (receipt?.ok !== true || receipt?.state !== 'recorded' || !liveSources.has(receipt?.skillSource) || receipt?.error) {
    return false;
  }

  const hasSession = Boolean(receipt.taskIdMasked || receipt.hermesSessionIdMasked);
  const hasOutputHash = /^[a-f0-9]{64}$/i.test(String(receipt.outputSha256 || ''));
  const selectedSkills = Array.isArray(receipt.selectedSkills) ? receipt.selectedSkills : [];

  if (receipt.skillSource === 'hermes-gateway') {
    return Boolean(receipt.taskIdMasked);
  }

  if (receipt.skillSource === 'nemohermes-sandbox') {
    return Boolean(
      hasSession &&
      receipt.sandboxId &&
      hasOutputHash &&
      selectedSkills.includes('governed-agentic-service-trial-v1')
    );
  }

  if (receipt.skillSource === 'hermes-cli') {
    return Boolean(
      hasSession &&
      hasOutputHash &&
      selectedSkills.includes('hermes-agent')
    );
  }
...
function normalizeHermesReceipt(result = {}) {
  ...
  return {
    ...receipt,
    receiptVerified: hasVerifiedHermesReceipt(receipt),
  };
}
