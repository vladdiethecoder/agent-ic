...
function evaluateProductionAccessDecision({ finalDecision, decision, trialEvidence, policyBlockResult, stripeResult, spendApproval, hermesExecutionReceipt }) {
  const blockers = [];
  ...
  const hermesRecorded = hermesExecutionReceipt?.ok === true;
  const liveSpendReceipt = Boolean(stripeResult?.checkout?.id) && stripeResult?.mode === 'live' && !String(stripeResult.checkout.id).startsWith('cs_test');
  ...
  if (spendApproval?.required !== true || spendApproval?.status !== 'approved') blockers.push('production spend/access approval missing');
