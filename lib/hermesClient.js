...
import { isHermesCliLive, isHermesNemoHermesLive, resolveHermesNemoHermesSandboxName, resolveHermesUrl } from './providerStatus.js';
...
const DETERMINISTIC_PLAYBOOK = {
  ...
  steps: [
    ...
    'Reference Stripe non-production envelope controls and official payment catalog skills for downstream payment execution.',
...
export async function dispatchToHermes(proposal, evaluation) {
  ...
  if (!url && isHermesCliLive()) {
    return dispatchToHostHermesCli(proposal, evaluation, start);
  }
...
async function dispatchToHostHermesCli(proposal, evaluation, start) {
  const prompt = buildNemoHermesPrompt(proposal, evaluation)
    .replace('Agent IC live Hermes dispatch', 'Agent IC local Hermes CLI dispatch')
    .replace('Use governed-agentic-service-trial-v1 for the selectedSkills value.', 'Use hermes-agent for the selectedSkills value and treat governed-agentic-service-trial-v1 as the local playbook artifact.');
  ...
  try {
    const result = await execFileClosedStdin(process.env.HERMES_BIN || 'hermes', [
      'chat',
      '-Q',
      '--pass-session-id',
      '--skills',
    ...
    const hermesSessionId = extractHermesSessionId(combined);
    const output = stripHermesSessionLine(combined);
    ...
    const taskId = hermesSessionId ? `hermes-cli-session-${hermesSessionId}` : `hermes-cli-${outputSha256.slice(0, 16)}`;
    return {
      ok: true,
      taskId,
      ...
      playbook: {
        ...
        id: hermesSessionId ? `hermes-cli-${hermesSessionId}` : taskId,
        executionSummary: summary,
      },
      skillSource: 'hermes-cli',
      provider: 'hermes-cli',
      command: 'hermes chat -Q --pass-session-id --skills hermes-agent',
      outputSha256,
...
function normalizeEvaluation(evaluation) {
  return {
    ...
    trialEvidence: evaluation.trialEvidence ? {
      casesProcessed: evaluation.trialEvidence.casesProcessed,
      autoRouted: evaluation.trialEvidence.autoRouted,
      humanReviewQueue: evaluation.trialEvidence.humanReviewQueue,
      dataHash: evaluation.trialEvidence.dataHash,
      blockedActionEnforced: evaluation.trialEvidence.blockedActionEnforced,
    } : undefined,
    playbook: evaluation.playbook ? {
      name: evaluation.playbook.name,
      version: evaluation.playbook.version,
      steps: evaluation.playbook.steps,
    } : undefined,
  };
}
