...
export async function runWorkerTrial({ caseDef, nemotronClient, onProgress }) {
  ...
  // If Nemotron is available, classify a small sample with the live provider,
  // then apply the deterministic taxonomy to the remaining rows and label that
  // portion as pattern-extended. If Nemotron is absent, malformed, or times out,
  // keep the trial usable with deterministic local classification, but label it
  // as fallback evidence rather than model output.
  const configuredSampleSize = Number(process.env.AGENT_IC_NEMOTRON_SAMPLE_SIZE || 3);
  const requestedSampleSize = Number.isFinite(configuredSampleSize)
    ? Math.min(12, Math.max(1, Math.floor(configuredSampleSize)))
    : 3;
  const sampleSize = nemotronClient?.available ? Math.min(requestedSampleSize, data.length) : 0;
  const sample = data.slice(0, sampleSize);
  let sampleResults = [];
  let fallbackReason = '';

  if (sampleSize > 0 && nemotronClient?.available) {
    onProgress?.({ phase: 'nemotron', message: `Classifying ${sampleSize} sample complaints with Nemotron...` });
    ...
    try {
      ...
      fallbackReason = `Nemotron unavailable for classification: ${String(error?.message || error).slice(0, 180)}`;
      onProgress?.({ phase: 'fallback', message: 'Nemotron unavailable; using labeled deterministic fallback classification.' });
      sampleResults = [];
    }
  } else {
    fallbackReason = 'Nemotron client unavailable; deterministic fallback classification used.';
    onProgress?.({ phase: 'fallback', message: 'Nemotron unavailable; using labeled deterministic fallback classification.' });
  }

  const results = fallbackReason
    ? data.map((item) => ({
        ...item,
        classification: extendClassificationPattern(item, domain),
        classifiedBy: 'deterministic-fallback',
        fallbackReason,
      }))
    : [
        ...sampleResults,
        ...data.slice(sampleSize).map((item) => {
  ...
  if (!fallbackReason) {
    onProgress?.({ phase: 'extend', message: `Extending classification to ${data.length - sampleSize} remaining items...` });
  }
  ...
  const evidence = computeEvidence(results, elapsed, domain, caseDef, { fallbackReason });
...
async function classifyWithNemotron(batch, domain, nemotronClient) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    const result = await nemotronClient.classify({
      ...
      temperature: 0,
      maxTokens: Number(process.env.NEMOTRON_CLASSIFY_MAX_TOKENS || Math.min(1200, Math.max(800, batch.length * 140))),
    });
  ...
  for (let index = 0; index < batch.length; index++) {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      const result = await nemotronClient.classify({
        ...
        model: process.env.NEMOTRON_MODEL || 'nvidia/nemotron-3-super-120b-a12b',
        temperature: 0,
        maxTokens: Number(process.env.NEMOTRON_CLASSIFY_MAX_TOKENS || 300),
      });
...
function buildClassificationPrompt(batch, domain) {
  ...
  const examples = batch.map((_, index) => ({ index, ...classificationExample(domain) }));
  return [
    instructions[domain] || instructions.safety,
    `Return only valid compact JSON with exactly ${batch.length} classifications in the same order.`,
    'Do not explain, reason, quote this prompt, use markdown, or add prose.',
    `Required JSON shape: ${JSON.stringify({ classifications: examples })}`,
    `Items: ${JSON.stringify(batch.map((item, index) => compactClassificationInput(item, index)))}`,
  ].join(' ');
}
...
function computeEvidence(results, elapsedMs, domain, caseDef, provenance = {}) {
  ...
  const nemotronClassified = results.filter((r) => r.classifiedBy === 'nemotron').length;
  const patternExtended = results.filter((r) => r.classifiedBy === 'pattern-extended').length;
  const deterministicClassified = results.filter((r) => r.classifiedBy === 'deterministic-fallback').length;
  const classificationMode = nemotronClassified > 0
    ? 'nemotron-sample-plus-pattern-extension'
  ...
  return {
    ...
    dataSourceMode: dataSourceMode(caseDef),
    dataSourceArtifact: caseDef.dataSource.localSnapshot || (String(caseDef.dataSource.url || '').startsWith('local ') ? caseDef.dataSource.url.replace(/^local\s+/, '') : null),
    ...
    classificationMethod: {
      ...
      unavailableReason: provenance.fallbackReason || null,
    },
  };
}

function dataSourceMode(caseDef) {
  if (caseDef.dataSource.localSnapshot && existsSync(caseDef.dataSource.localSnapshot)) return 'checked-in-snapshot';
  if (String(caseDef.dataSource.url || '').startsWith('local ')) return 'checked-in-fixture';
  return 'live-public-api';
}
