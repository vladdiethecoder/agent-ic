 ...
 *
 * Uses a local JSON file store (no external DB dependency for the hackathon).
 */

import { existsSync, readFileSync, writeFileSync, mkdirSync } from 'node:fs';
import { join } from 'node:path';

const LEDGER_DIR = join(process.cwd(), '.agent-ic', 'ledger');
const LEDGER_FILE = join(LEDGER_DIR, 'evidence-ledger.json');

function ensureLedger() {
  if (!existsSync(LEDGER_DIR)) mkdirSync(LEDGER_DIR, { recursive: true });
  if (!existsSync(LEDGER_FILE)) {
    writeFileSync(LEDGER_FILE, JSON.stringify({ cycles: [], vendors: {} }, null, 2));
  }
}
...
function computeRenewalRecommendation(ledger, caseId, vendorKey) {
  const vendorCycles = ledger.cycles.filter(
    (c) => c.caseId === caseId && (c.vendor?.name === vendorKey || true)
  );
 ...
 */
export function getRenewalHistory(caseId) {
  const ledger = readLedger();
...
/**
 * Seed the ledger with multi-cycle history for demo purposes.
 * Creates 3 cycles showing progression: trial → expand → renew.
 */
export function seedDemoRenewalHistory(caseId, caseDef) {
  const ledger = readLedger();

  // Don't seed if history already exists
  if (ledger.cycles.some((c) => c.caseId === caseId)) {
    return getRenewalHistory(caseId);
  }
 ...
 */
export function getAllVendorRelationships() {
  const ledger = readLedger();
  const relationships = [];
