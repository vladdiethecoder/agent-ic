...
export function activatePolicyVersion({ tenantId, policyId, activatedBy }) {
  ...
  const evaluation = evaluatePolicy({ policy: policy.policy, attemptedAction: policy.policy.blockedTool });
  if (!evaluation.blocked) return { ok: false, code: 'policy_evaluation_failed', message: 'Policy cannot be activated because blocked tool evaluation did not deny' };
...
export function evaluatePolicy({ policy, attemptedAction = {} }) {
