...
function buildTrialRecord({ tenantId, userId, result }) {
  ...
  return {
    ...
    roiMethodology: result.roiMethodology ? {
      baseline: result.roiMethodology.baseline,
      agent: result.roiMethodology.agent,
      computed: result.roiMethodology.computed,
      measurementNote: result.roiMethodology.measurementNote,
    } : undefined,
