...
import { scoreProposal } from './decisionEngine.js';
import { seededProposals } from './demoData.js';
...
export function computeSpendCapDollars(evaluation) {
  ...
  return Math.max(
    1000,
    Math.round((evaluation.autonomousSpendCap || evaluation.recommendedBudget * 0.12) / 100) * 100
  );
}

export function buildMockCheckoutSession(proposal, spendCapDollars) {
  const amountCents = spendCapDollars * 100;
  return {
    id: `cs_test_agent_ic_${proposal.id}_${Date.now()}`,
    ...
    currency: 'usd',
    url: `/mock-stripe-checkout?session=cs_test_agent_ic_${proposal.id}`,
    client_reference_id: proposal.id,
    metadata: {
      proposal_id: proposal.id,
      governance_policy: 'NemoClaw/OpenShell-style envelope',
      autonomous_spend_cap_dollars: String(spendCapDollars),
    },
  };
}

export async function createCheckoutSession(request, proposal, evaluationOverride = null, opts = {}) {
  const evaluation = evaluationOverride || scoreProposal(proposal);
  ...
  if (demoMode) {
    const checkout = buildMockCheckoutSession(proposal, spendCapDollars);
    return { mode: 'demo', checkout, spendCapDollars, evaluation };
  }
  ...
  params.set('metadata[governance_policy]', 'NemoClaw/OpenShell-style envelope');
  ...
  params.set('line_items[0][price_data][product_data][name]', `Agent IC governed pilot authorization — ${proposal.company}`);
  params.set(
