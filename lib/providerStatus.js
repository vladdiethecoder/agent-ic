...
export function buildProviderStates() {
  ...
  return {
    ...
    hermes: {
      state: (hermesUrlConfigured || hermesSandboxLive) && !isDemoModeGlobal() ? 'live' : 'mock',
      mode: (hermesUrlConfigured || hermesSandboxLive) && !isDemoModeGlobal() ? 'live' : 'demo',
      provider: hermesSandboxLive ? 'nemohermes-sandbox' : hermesUrlConfigured ? 'hermes-gateway' : 'local-artifact',
      detail:
        (hermesUrlConfigured || hermesSandboxLive) && !isDemoModeGlobal()
          ? undefined
          : isDemoModeGlobal()
            ? 'demo mode enabled'
    ...
    nemoclaw: {
      state: policyExternalLive ? 'live' : 'mock',
      mode: policyExternalLive ? 'live' : 'demo',
