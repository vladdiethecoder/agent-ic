...
const LOCAL_DEV_PRINCIPAL = Object.freeze({
  userId: 'local-user',
...
export function principalFromRequest(request, { allowLocalDev = true, env = process.env } = {}) {
  ...
  if (!isProductionMode(env) && allowLocalDev) {
...
export async function principalFromRequestAsync(request, { allowLocalDev = true, env = process.env } = {}) {
  const bearer = bearerToken(request.headers.get('authorization'));
  if (bearer) {
  ...
  return principalFromRequest(request, { allowLocalDev, env });
}
