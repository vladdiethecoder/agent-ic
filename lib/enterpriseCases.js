 ...
 *   - Natural policy block (OpenShell-enforced when observed; otherwise explicitly local policy-gate)
...
export const GOVERNANCE_INVARIANTS = [
  ...
  'Worker agent is governed by an observed OpenShell receipt when present; otherwise by an explicitly labeled local deny-by-default policy gate.',
...
const safetyOpsCase = {
  ...
  roiMethodology: {
    ...
    agent: {
      ...
      inputs: {
        humanReviewCases: { source: 'measured trial evidence', value: 29, unit: 'complaints' },
        ...
        governedEnvelopeSpend: { source: 'Stripe Checkout receipt', value: 100, unit: 'USD' },
      },
      result: { totalCost: 367, unit: 'USD', agentHours: 2.9 },
    },
    computed: {
      hoursSaved: { formula: 'baseline.totalHours - agent.agentHours', value: 30.1, unit: 'hours' },
      productivityLift: { formula: 'casesPerHourAgent ÷ casesPerHourManual', value: 11.4, unit: 'x' },
      costPerCaseBaseline: { formula: 'baseline.totalCost ÷ cases', value: 9.2, unit: 'USD/case' },
      costPerCaseAgent: { formula: 'agent.totalCost ÷ cases', value: 1.11, unit: 'USD/case' },
      netValue: { formula: 'baseline.totalCost - agent.totalCost', value: 2669, unit: 'USD' },
    },
    assumptions: [
      ...
      'Human review applies to manual_review + critical_review queues; technical and safety_review cases are routed with audit trace.',
...
const engineeringCase = {
  ...
  dataSource: {
    ...
    authRequired: false,
    authNote: 'Set GITHUB_TOKEN env for higher rate limits. Unauthenticated requests work for local development.',
  },
  ...
  evidencePlan: [
    ...
    'Block any write action (merge, push, comment-posting) with OpenShell when observed or the local policy gate otherwise',
