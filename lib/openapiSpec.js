  ...
  return {
    ...
    components: {
      ...
      schemas: {
        ...
        RoiComputedMetric: objectSchema({
          formula: { type: 'string' },
          value: { type: 'number' },
          unit: { type: 'string' },
        }, ['formula', 'value']),
        RoiMethodology: objectSchema({
          baseline: objectSchema({
            inputs: { type: 'object', additionalProperties: true },
            result: objectSchema({ totalCost: { type: 'number' } }, ['totalCost']),
          }, ['inputs', 'result']),
          agent: objectSchema({
            inputs: { type: 'object', additionalProperties: true },
            result: objectSchema({ totalCost: { type: 'number' } }, ['totalCost']),
          }, ['inputs', 'result']),
          computed: objectSchema({
            netValue: { $ref: '#/components/schemas/RoiComputedMetric' },
          }, ['netValue']),
          measurementNote: { type: 'string' },
        }, ['baseline', 'agent', 'computed']),
        AllowedActionProof: objectSchema({
        ...
        PolicyBlockResult: objectSchema({
          blocked: { type: 'boolean', const: true },
          status: { type: 'integer', enum: [403] },
          allowedAction: { $ref: '#/components/schemas/AllowedActionProof' },
          enforcementEngine: { type: 'string' },
          enforcementMode: { type: 'string' },
          verificationStatus: { type: 'string' },
          attemptedAmount: { type: 'number' },
          cap: { type: 'number' },
          policyRule: { type: 'string' },
        }, ['blocked', 'status', 'allowedAction']),
        TrialRun: objectSchema({
          ...
          policyBlock: objectSchema({ result: { $ref: '#/components/schemas/PolicyBlockResult' } }, ['result']),
          roiMethodology: { $ref: '#/components/schemas/RoiMethodology' },
        }, ['runId', 'caseId', 'spendApproval', 'productionAccessDecision', 'roiMethodology']),
        ...
        EnterpriseTrialResult: objectSchema({
          ...
          decision: { type: 'object', additionalProperties: true },
          roiMethodology: { $ref: '#/components/schemas/RoiMethodology' },
        }, ['runId', 'caseId', 'spendApproval', 'productionAccessDecision', 'policyBlock', 'roiMethodology']),
