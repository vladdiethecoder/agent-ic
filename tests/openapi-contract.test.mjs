...
test('OpenAPI spec validates current API contract foundation', async () => {
  ...
  assert.ok(spec.components.schemas.RoiMethodology, 'ROI methodology schema is documented');
  assert.ok(spec.components.schemas.PolicyBlockResult, 'policy block result schema is documented');
  assert.ok(spec.components.schemas.StrictProviderProofError, 'strict provider proof error schema is documented');
  assert.equal(spec.components.schemas.EnterpriseTrialResult.required.includes('roiMethodology'), true);
  assert.equal(spec.components.schemas.EnterpriseTrialResult.required.includes('policyBlock'), true);
  assert.equal(spec.components.schemas.EnterpriseTrialResult.properties.roiMethodology.$ref, '#/components/schemas/RoiMethodology');
  assert.equal(spec.components.schemas.EnterpriseTrialResult.properties.policyBlock.properties.result.$ref, '#/components/schemas/PolicyBlockResult');
  assert.equal(spec.components.schemas.TrialRun.required.includes('roiMethodology'), true);
  assert.equal(spec.components.schemas.TrialRun.properties.policyBlock.properties.result.$ref, '#/components/schemas/PolicyBlockResult');
  assert.equal(spec.components.schemas.TrialRun.properties.roiMethodology.$ref, '#/components/schemas/RoiMethodology');
