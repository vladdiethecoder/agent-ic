...
function trialResult({ tenantId, caseDef, verdict, runId, cap = 100, netValue = 1000, policyEnforced = true }) {
  return {
    ...
    productionAccessDecision: {
      ...
      blockers: verdict === 'CONTINUE' ? ['production spend/access approval missing'] : [`procurement verdict is ${verdict}`],
    },
    roiMethodology: {
      baseline: { inputs: { cases: { value: 100 } }, result: { totalCost: 1200 } },
      agent: { inputs: { humanReviewCases: { value: 20 } }, result: { totalCost: 1200 - netValue } },
      computed: { netValue: { formula: 'baseline.totalCost - agent.totalCost', value: netValue, unit: 'USD' } },
    },
  };
}
