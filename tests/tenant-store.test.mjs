...
function minimalRoiMethodology({ baseline = 1200, agent = 200 } = {}) {
  return {
    baseline: { inputs: { cases: { value: 10 } }, result: { totalCost: baseline } },
    agent: { inputs: { humanReviewCases: { value: 2 } }, result: { totalCost: agent } },
    computed: { netValue: { formula: 'baseline.totalCost - agent.totalCost', value: baseline - agent, unit: 'USD' } },
  };
}
...
test('trial store persists completed run and isolates tenants', async () => {
  ...
  const result = {
    ...
    roiMethodology: minimalRoiMethodology({ baseline: 3036, agent: 532 }),
