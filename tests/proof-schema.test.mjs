...
test('enterprise trial result schema requires production-access proof artifacts', () => {
  const base = {
    ...
    policyBlock: { result: { blocked: true, status: 403, allowedAction: { decision: 'allowed', status: 200, tool: 'NHTSA evidence read' } } },
  ...
  assert.equal(parseSchema(EnterpriseTrialResultSchema, {
    ...base,
    policyBlock: { result: { blocked: false, status: 200, allowedAction: { decision: 'allowed', status: 200 } } },
  }).ok, false);
  const missingRoi = { ...base };
  delete missingRoi.roiMethodology;
  assert.equal(parseSchema(EnterpriseTrialResultSchema, missingRoi).ok, false);
  assert.equal(parseSchema(EnterpriseTrialResultSchema, {
    ...base,
    roiMethodology: {
      ...base.roiMethodology,
      computed: { netValue: { formula: 'baseline.totalCost - agent.totalCost', value: 1, unit: 'USD' } },
    },
  }).ok, false);
});
...
test('strict provider proof error schema carries missing proof blockers', () => {
  const error = {
    error: 'Strict provider proof required but missing: Stripe live-mode Checkout receipt, Hermes dispatch receipt',
    code: 'strict_provider_proof_missing',
    decision: 'blocked',
    missingProof: ['Stripe live-mode Checkout receipt', 'Hermes dispatch receipt'],
  };
...
test('renewal cycle schema requires approval and production-access proof artifacts', () => {
  ...
  const missingRoi = { ...cycle };
  delete missingRoi.roiMethodology;
  assert.equal(parseSchema(RenewalCycleRecordSchema, missingRoi).ok, false);
});
