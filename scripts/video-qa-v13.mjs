...
const videoPath = process.env.AGENT_IC_DEMO_VIDEO || 'demo-out/agent-ic-demo-v13.mp4';
const frameDir = process.env.AGENT_IC_QA_FRAME_DIR || 'demo-out/qa-frames-v13';
const reportPath = process.env.AGENT_IC_QA_REPORT || 'demo-out/video-qa-report-v13.json';
const skipOcr = process.env.AGENT_IC_QA_SKIP_OCR === 'true';
const requiredText = ['CONTINUE', '$185,000', '403', 'Hermes', 'Stripe', 'NemoClaw'];
...
async function readSourceText() {
  let text = '';
  const files = [
    'remotion/edit-plan-v13.json',
    'remotion/src/captions-v13.json',
    'demo-out/video-text-manifest-v13.txt',
  ];
...
async function loadEditPlan() {
  try {
    const raw = await fs.readFile('remotion/edit-plan-v13.json', 'utf8');
...
async function main() {
  ...
  // Reject any in-frame "SIMULATED" watermark left over from earlier cuts.
  const hasSimulatedBadge = /SIMULATED/i.test(combinedText);
  check('No in-frame SIMULATED badge', !hasSimulatedBadge, hasSimulatedBadge ? 'found SIMULATED' : 'clean');
