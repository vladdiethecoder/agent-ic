...
const videoPath = process.env.AGENT_IC_DEMO_VIDEO || 'demo-out/agent-ic-demo-v12.mp4';
const frameDir = process.env.AGENT_IC_QA_FRAME_DIR || 'demo-out/qa-frames-v12';
const reportPath = process.env.AGENT_IC_QA_REPORT || 'demo-out/video-qa-report-v12.json';
const skipOcr = process.env.AGENT_IC_QA_SKIP_OCR === 'true';
const requiredText = ['CONTINUE', '$185,000', '403 Forbidden'];
...
async function readSourceText() {
  let text = '';
  const files = ['remotion/edit-plan-v12.json', 'remotion/src/captions-v12.json'];
