...
function loadPolicy() {
  ...
  try {
    ...
    console.error(`Failed to load NemoClaw policy from ${policyPath}: ${error.message}`);
    process.exit(1);
  }
}
...
async function main() {
  ...
  const port = Number(new URL(proxyUrl).port) || 9000;
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      console.error('NemoClaw proxy error:', error);
      return sendJson(response, 500, { error: 'internal_error', detail: error.message });
    }
  });

  server.listen(port, hostname, () => {
    console.log(`NemoClaw policy proxy listening on ${hostname}:${port}`);
...
main().catch((error) => {
  console.error('NemoClaw proxy failed to start:', error);
  process.exit(1);
});
