#!/usr/bin/env node
/**
 * Idempotent installer for Agent IC v13 demo CLI tooling.
 *
 * Attempts to install ttyd, vhs, and the Stripe CLI into the project-local
 * `tools/` directory (preferred) or `~/.local/bin`. NemoClaw / Hermes CLIs are
 * not publicly distributable; the script detects them and reports whether real
 * capture is possible.
 *
...
import { mkdir, chmod, rm, writeFile, access, readFile } from 'node:fs/promises';
...
const REPORT_FILE = resolve(REPORT_DIR, 'tool-install-report-v13.json');
...
async function installBinary(toolName, targetDir, force = false) {
  ...
  if (!force) {
    try {
      ...
      const versionResult = spawnSync(destPath, ['--version'], { encoding: 'utf8', stdio: 'pipe' });
...
async function detectCli(name, installUrl) {
  ...
  return {
    ...
    note: exists
      ? 'Configure the matching environment variable to enable real capture.'
      : `Not installable by this script. See ${installUrl}. Real capture will use replay fallback.`,
  };
}
...
async function main() {
  ...
  for (const item of report.items) {
    console.log(`  ${item.name}: ${item.status}${item.path ? ` → ${item.path}` : ''}${item.reason ? ` — ${item.reason}` : ''}${item.note ? ` — ${item.note}` : ''}`);
  }

  const failures = report.items.filter((i) => i.status === 'failed');
  if (failures.length) {
    console.warn('\nSome tools failed to install. The capture script will fall back to Playwright replay for those sessions.');
  }
}
