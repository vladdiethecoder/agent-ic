...
const VOICE_IN = process.argv[2] || 'remotion/public/voiceover-v11.wav';
const OUTPUT_MASTER = process.argv[3] || 'demo-out/agent-ic-audio-mastered-v12.wav';
const OUTPUT_PUBLIC = process.argv[4] || 'remotion/public/agent-ic-audio-mastered-v12.wav';
const ASSETS_DIR = process.argv[5] || 'demo-out/audio-assets';
...
const DEFAULT_SFX_CUES = [
  ...
  { id: 'eval-success',    file: 'sfx-eval-success.wav',    offset: 50.0, duration: 0.6,  volume: -12 },
  { id: 'stripe-auth',     file: 'sfx-stripe-auth.wav',     offset: 65.0, duration: 0.4,  volume: -10 },
  { id: 'policy-block',    file: 'sfx-policy-block.wav',    offset: 95.0, duration: 0.55, volume: -10 },
  { id: 'outro-whoosh',    file: 'sfx-outro-whoosh.wav',    offset: 115.0,duration: 1.2,  volume: -9 },
];

function run(label, cmd, args, options = {}) {
  console.log(`[audio-v12] ${label}: ${cmd} ${args.join(' ')}`);
  ...
  if (result.status !== 0) {
    ...
    console.error(`[audio-v12] FAILED ${label}: ${err.slice(0, 1200)}`);
...
function main() {
  console.log('[audio-v12] Starting Agent IC v12 audio mastering');
  console.log(`[audio-v12] Input: ${VOICE_IN}`);

  ensureDir(path.dirname(OUTPUT_MASTER));
  ensureDir(ASSETS_DIR);

  if (!fs.existsSync(VOICE_IN)) {
    console.error(`[audio-v12] Voiceover not found: ${VOICE_IN}`);
    process.exit(1);
  }

  const probe = probeVoice(VOICE_IN);
  if (!probe || !Number.isFinite(probe.duration)) {
    console.error('[audio-v12] Could not probe voiceover duration');
    process.exit(1);
  }
  console.log(`[audio-v12] Voiceover: ${probe.duration.toFixed(3)}s, ${probe.sampleRate} Hz, ${probe.channels} ch`);
  ...
  if (!m) {
    console.error('[audio-v12] Could not parse loudnorm measurements');
    process.exit(1);
  }
  console.log('[audio-v12] Loudnorm measurements:', m);
  ...
  if (!mixM) {
    console.error('[audio-v12] Could not parse raw mix loudness measurements');
    process.exit(1);
  }
  console.log('[audio-v12] Raw mix loudness measurements:', mixM);
  ...
  console.log(`[audio-v12] Copied mastered audio to ${OUTPUT_PUBLIC}`);

  // QA loudness.
  const loudness = probeLoudness(OUTPUT_MASTER);
  if (loudness) {
    console.log('[audio-v12] Final loudness:', loudness);
    ...
    if (Number.isFinite(i)) {
      if (i < TARGET_I - 1.5 || i > TARGET_I + 1.5) {
        console.warn(`[audio-v12] WARNING: integrated loudness ${i} LUFS outside ±1.5 LU target`);
      } else {
        console.log(`[audio-v12] Integrated loudness: ${i} LUFS ✓`);
      }
    }
    if (Number.isFinite(tp)) {
      if (tp > TARGET_TP + 0.1) {
        console.warn(`[audio-v12] WARNING: true peak ${tp} dBTP exceeds ${TARGET_TP}`);
      } else {
        console.log(`[audio-v12] True peak: ${tp} dBTP ✓`);
      }
    }
    if (Number.isFinite(lra)) {
      console.log(`[audio-v12] Loudness range: ${lra} LU`);
    }
  } else {
    console.warn('[audio-v12] Could not probe final loudness');
  }

  console.log('[audio-v12] Done.');
}
