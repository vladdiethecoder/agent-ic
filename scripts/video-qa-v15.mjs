...
const videoPath = process.env.AGENT_IC_DEMO_VIDEO || 'demo-out/agent-ic-demo-v15.mp4';
const frameDir = process.env.AGENT_IC_QA_FRAME_DIR || 'demo-out/qa-frames-v15';
const reportPath = process.env.AGENT_IC_QA_REPORT || 'demo-out/video-qa-report-v15.json';
...
async function readSourceText() {
  let text = '';
  const files = [
    'remotion/edit-plan-v15.json',
    'remotion/src/captions-v15.json',
    'demo-out/video-text-manifest-v15.txt',
  ];
...
async function loadEditPlan() {
  try {
    const raw = await fs.readFile('remotion/edit-plan-v15.json', 'utf8');
...
async function loadCursorEvents() {
  try {
    const raw = await fs.readFile('demo-out/cursor-events-v15.json', 'utf8');
...
async function loadTerminalReport() {
  try {
    const raw = await fs.readFile('demo-out/terminals-v15/capture-report-v15.json', 'utf8');
    return JSON.parse(raw);
  } catch {
    return null;
  }
...
async function main() {
  ...
  if (terminalReport && terminalReport.sourceDir) {
    check(
      'Terminal clips rendered from v15 source directory',
      terminalReport.sourceDir.includes('terminals-v15'),
      terminalReport.sourceDir
    );
  } else {
    check('Terminal clips rendered from v15 source directory', false, 'missing capture-report-v15.json');
  }
