#!/usr/bin/env node
/**
 * Agentic editing planner for Agent IC v7.
 *
 * Reads the voiceover script, stage timestamps from the base recording, and
 * the orchestration payload, then produces a deterministic JSON edit plan.
 ...
 *
 * Output: remotion/edit-plan.json
 */
...
const scriptPath = process.argv[2] || 'demo/voiceover-v7.txt';
const timestampsPath = process.argv[3] || 'remotion/stage-timestamps.json';
const payloadPath = process.argv[4] || 'remotion/src/payload.json';
const audioPath = process.argv[5] || 'remotion/public/voiceover.wav';
const outputPath = process.argv[6] || 'remotion/edit-plan.json';
...
function keywordStageId(sentence) {
  ...
  const rules = [
    ...
    ['nemotron', 'mission'],
    ['mission', 'mission'],
    ['intake skill', 'mission'],
    ['stripe link skill', 'stripeSkill'],
...
async function buildPlan() {
  ...
  const audioDuration = ffprobeDuration(audioPath) || 83;
  const mainFrames = Math.max(Math.round(audioDuration * FPS), FPS * 60);
  const totalFrames = INTRO_FRAMES + mainFrames + OUTRO_FRAMES;

  const defaultStages = [
    { id: 'hero', label: 'Hero / pre-run', frame: 0 },
    { id: 'mission', label: 'Mission loaded', frame: FPS * 12 },
    { id: 'sandbox', label: 'OpenShell sandbox ready', frame: FPS * 22 },
    { id: 'stripe', label: 'Stripe authorization recorded', frame: FPS * 30 },
    { id: 'stripeSkill', label: 'Stripe Link skill approved', frame: FPS * 38 },
    { id: 'blocked', label: 'Out-of-policy spend blocked', frame: FPS * 48 },
    { id: 'evidence', label: 'Evidence imported', frame: FPS * 58 },
    { id: 'skills', label: 'Hermes skills executed', frame: FPS * 66 },
    { id: 'decision', label: 'Capital decision issued', frame: FPS * 74 },
    { id: 'counterfactual', label: 'Counterfactual KILL', frame: Math.max(FPS * 78, mainFrames - FPS * 4) },
  ];
  ...
  // Any remaining nulls at the start default to hero.
  deterministicMappings = deterministicMappings.map((stageId) => stageId || 'hero');
  ...
  const callouts = [
    {
      ...
      endFrame: stageFrame('sandbox') - FPS,
      text: payload.mission?.description ? `Mission: ${payload.mission.description}` : 'Mission loaded',
    },
    {
      stageId: 'sandbox',
      startFrame: stageFrame('sandbox'),
      endFrame: stageFrame('stripe') - FPS,
      text: 'OpenShell sandbox: deny-all except allow-listed tool endpoints',
    },
    {
      stageId: 'stripe',
      startFrame: stageFrame('stripe'),
      endFrame: stageFrame('blocked') - FPS,
      text: `Stripe authorization: ${payload.envelope?.cap ? `$${payload.envelope.cap} cap` : 'bounded cap'} · recurring spend blocked`,
    },
    {
      stageId: 'stripeSkill',
      startFrame: stageFrame('stripeSkill'),
      endFrame: stageFrame('blocked') - FPS,
      text: `${payload.stripeSkill?.displayName || 'Stripe Link skill'}: ${payload.stripeSkill?.action || 'Buy SaaS subscription with one-time virtual card'}`,
    },
    {
      stageId: 'blocked',
      startFrame: stageFrame('blocked'),
      endFrame: stageFrame('evidence') - FPS,
      text: `OpenShell blocks ${payload.blocked?.attemptedTool || 'out-of-policy API call'} — 403 Forbidden`,
    },
    {
      stageId: 'evidence',
      startFrame: stageFrame('evidence'),
      endFrame: stageFrame('skills') - FPS,
    ...
    {
      stageId: 'skills',
      startFrame: stageFrame('skills'),
      endFrame: stageFrame('decision') - FPS,
      text: 'Hermes skills: auto-order parts & notify Slack within the envelope',
    },
    {
      stageId: 'decision',
      startFrame: stageFrame('decision'),
      endFrame: stageFrame('counterfactual') - FPS,
    ...
    },
    {
      ...
      endFrame: mainFrames,
      text: 'Drop QA to 82% and the decision flips to KILL',
    },
