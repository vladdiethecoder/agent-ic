...
async function main() {
  ...
  const evidence = trial.workerResult?.evidence || {};
  ...
  assert(evidence.classificationMethod?.nemotronClassified > 0, 'Nemotron sample classifications');
  assert(evidence.classificationMethod?.patternExtended > 0, 'pattern extension tracked');
  ...
  console.log(JSON.stringify({
    ...
    nemotronClassified: evidence.classificationMethod.nemotronClassified,
    patternExtended: evidence.classificationMethod.patternExtended,
