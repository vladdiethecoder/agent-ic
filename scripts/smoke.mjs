...
async function main() {
  ...
  assert(health.providerStates?.nemotron?.state === 'configured', 'Nemotron configured without claiming per-run success');
  assert(health.providerStates?.stripe?.state === 'configured', 'Stripe configured without claiming per-run success');
  assert(/per-run receipts/i.test(health.truthModel || ''), 'health truth model requires per-run receipts');

  const ready = await json(`${base}/api/ready`);
  assert(ready.status === 'ready', 'readiness endpoint ready');
  assert(ready.productionReady === false, 'development readiness is not production-ready claim');
  ...
  const latestProof = await json(`${base}/api/proof-report`);
  assert(latestProof.latestTrial?.runId === trial.runId, 'proof report latest trial matches executed trial');
  assert(latestProof.latestTrial?.productionAccessDecision?.approved === false, 'proof report carries production access denial by default');
  assert(Array.isArray(latestProof.latestTrial?.productionAccessDecision?.blockers), 'proof report carries production access blockers');
  ...
  console.log(JSON.stringify({
    ...
    proofLatestTrial: latestProof.latestTrial.runId,
    productionAccessApproved: latestProof.latestTrial.productionAccessDecision.approved,
  }, null, 2));
}
