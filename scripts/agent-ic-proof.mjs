...
export function classifyStripeProof(stripe = {}) {
  ...
  const liveReceipt = hasCheckoutSession && !nonProductionReceipt && (receiptText.startsWith('cs_live') || stripe.mode === 'live');
  return {
    stripeReceiptId,
    hasCheckoutSession,
    ...
    liveReceipt,
    proof: liveReceipt
      ? 'Checkout Session create recorded in live mode'
...
async function proofProductionAccess() {
  ...
  if (!decision || typeof decision.approved !== 'boolean' || !decision.status || !decision.scope) {
    throw new Error('Production-access decision proof is missing from trial result');
  }
  const blockers = Array.isArray(decision.blockers) ? decision.blockers : [];
  if (decision.approved === true && blockers.length > 0) {
    throw new Error('Production-access approval cannot include blockers');
  }
  if (decision.approved === false && blockers.length === 0) {
    throw new Error('Production-access denial requires at least one blocker');
  }
  ...
  print({
    ...
    receiptEvidence: {
      ...
      hermesReceipt: decision.evidence?.hermesReceipt === true,
