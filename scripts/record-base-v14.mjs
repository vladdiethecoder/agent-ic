...
try {
  ...
  const finalVideo = path.join(outDir, 'ui-v14.webm');
  const publicVideo = path.join(path.resolve('remotion/public'), 'ui-v14.webm');
  ...
  try {
    const tempTranscoded = path.join(rawDir, `ui-v14-30fps-${Date.now()}.webm`);
  ...
  const timestampsPath = path.join(outDir, 'stage-timestamps-v14.json');
  ...
  await fs.copyFile(timestampsPath, path.join(remotionPublic, 'stage-timestamps-v14.json'));

  const cursorPath = path.join(outDir, 'cursor-events-v14.json');
  await fs.writeFile(
    cursorPath,
    JSON.stringify(
      {
  ...
  await fs.copyFile(cursorPath, path.join(remotionPublic, 'cursor-events-v14.json'));
