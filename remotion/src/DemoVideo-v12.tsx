...
function StageLabelPill({
  ...
  const end = endFrame;
  const fade = Math.min(10, Math.max(1, Math.floor((end - start) / 5)));
  ...
  const translateX = interpolate(layer, [start - fade, start], [-40, 0], {
...
function HighlightRing({
  ...
  const pulse = interpolate(layer, [start, end], [0, 6], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  const ringScale = 1 + (pulse % 1) * 0.35;
  const ringOpacity = 1 - (pulse % 1);
