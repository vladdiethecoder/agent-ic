...
Start with `JUDGE_QUICKSTART.md`, then watch the X-attached demo video:
...
The stripped public repo intentionally excludes generated videos, frame dumps, browser profiles, local state, and `.env.local`; the video should be attached to the public X submission post. `SUBMISSION_MANIFEST.json`, `SUBMISSION.md`, `FINAL_SUBMISSION_PACKET.md`, and `VALIDATION.md` contain the proof map, current validation status, tweet copy, Typeform copy, and fail-closed QA rules.
