...
The stripped public repo intentionally excludes generated videos, frame dumps, browser profiles, local state, and `.env.local`; the video must be attached to the public X submission post. The GitHub release asset bundle is a fallback/audit copy of the same MP4, QA reports, contact sheet, sidecar, cover, and stripped source tarball. `VIDEO_JUDGE_GUIDE.md`, `JUDGE_SCORECARD.md`, `SUBMISSION_MANIFEST.json`, `SUBMISSION.md`, `FINAL_SUBMISSION_PACKET.md`, and `VALIDATION.md` contain the timestamped video guide, criteria map, proof map, current validation status, tweet copy, Typeform copy, and fail-closed QA rules.
